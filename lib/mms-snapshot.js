...
MmsSnap.prototype.exec = function(cb) {
	var self = this;
	self.getSnapshotList(function(err, snap) {
		if (err) {
			return cb(err);
		}
		if (!snap.complete) {
		...
		self.createRestoreJob(snap.id, function(err, rjob) {
			...
			self.getSnapshot(rjob.id, function(err, sUrl) {
				if (err) {
					return cb(err);
				}
				if (self.verbose) {
...
MmsSnap.prototype.getSnapshotList = function(cb) {
	var self = this;
...
MmsSnap.prototype.getSnapshot = function(restoreId, cb) {
	var self = this;
...
MmsSnap.prototype.download = function(url, cb) {
	...
	req.on('data', function() {}).pipe(file);
